...
// node_modules/vitepress-plugin-legend/dist/blockDiagram-6J76NXCF-CKU6xA4s-ur9z755R.js
var xt = (function() {
  var e2 = F(function(v, c, p, w) {
    ...
    performAction: F(function(v, c, p, w, E, d, _) {
      var y2 = d.length - 1;
      switch (E) {
        case 4:
    ...
    parse: F(function(v) {
      var c = this, p = [0], w = [], E = [null], d = [], _ = this.table, y2 = "", A = 0, O = 0, q = 2, j = 1, ie = d.slice.call(arguments, 1), z = Object.create(this.lexer), V = { yy: {} };
      ...
      F(Dt, "lex");
      for (var P, G, X, ut, J = {}, at, Z, vt, st; ; ) {
        if (G = p[p.length - 1], this.defaultActions[G] ? X = this.defaultActions[G] : ((P === null || typeof P > "u") && (P = Dt()), X = _[G] && _[G][P]), typeof X > "u" || !X.length || !X[0]) {
          var yt = "";
          st = [];
          for (at in _[G])
...
Expecting ` + st.join(", ") + ", got '" + (this.terminals_[P] || P) + "'" : yt = "Parse error on line " + (A + 1) + ": Unexpected " + (P == j ? "end of input" : "'" + (this.terminals_[P] || P) + "'"), this.parseError(yt, {
        ...
        switch (X[0]) {
          case 1:
            ...
            break;
          case 2:
            if (Z = this.productions_[X[1]][1], J.$ = E[E.length - Z], J._$ = {
              ...
              E,
              d
            ].concat(ie)), typeof ut < "u")
              return ut;
            Z && (p = p.slice(0, -1 * Z * 2), E = E.slice(0, -1 * Z), d = d.slice(0, -1 * Z)), p.push(this.productions_[X[1]][0]), E.push(J.$), d.push(J._$), vt = _[p[p.length - 2]][p[p.length - 1]], p.push(vt);
            break;
          case 3:
            return true;
        }
...
` + p + "^";
      ...
      next: F(function() {
        ...
        for (var d = this._currentRules(), _ = 0; _ < d.length; _++)
          if (w = this._input.match(this.rules[d[_]]), w && (!p || w[0].length > p[0].length)) {
            if (p = w, E = _, this.options.backtrack_lexer) {
              if (c = this.test_match(w, d[_]), c !== false)
                return c;
              if (this._backtrack) {
                p = false;
                continue;
...
` + this.showPosition(), {
      ...
      options: {},
      performAction: F(function(c, p, w, E) {
        switch (w) {
          case 0:
            return 10;
          case 1:
  ...
  return F(D, "Parser"), D.prototype = k, k.Parser = D, new D();
})();
xt.parser = xt;
var ke = xt;
...
var De = F((e2) => yu.sanitizeText(e2, Ee), "sanitizeText");
var ve = F(function(e2, t = "") {
  let a = lt.get(e2);
  a || (a = { id: e2, styles: [], textStyles: [] }, lt.set(e2, a)), t?.split(Pt).forEach((n2) => {
    const i = n2.replace(/([^;]*);/, "$1").trim();
    if (RegExp($t).exec(n2)) {
      const r = i.replace(Tt2, _e).replace($t, Tt2);
      a.textStyles.push(r);
    }
    a.styles.push(i);
  });
}, "addStyleClass");
var Ne = F(function(e2, t = "") {
  const a = F2.get(e2);
  t != null && (a.styles = t.split(Pt));
}, "addStyle2Node");
var $e = F(function(e2, t) {
  e2.split(",").forEach(function(a) {
    let n2 = F2.get(a);
    if (n2 === void 0) {
      const i = a.trim();
      n2 = { id: i, type: "na", children: [] }, F2.set(i, n2);
    }
...
var Yt = F((e2, t) => {
  ...
  for (const i of a) {
    ...
    if (i.type === "applyStyles") {
      i?.stylesStr && Ne(i.id, i?.stylesStr);
      continue;
    }
    if (i.type === "column-setting")
      ...
      const r = F2.get(i.id);
      if (r === void 0 ? F2.set(i.id, i) : (i.type !== "na" && (r.type = i.type), i.label !== i.id && (r.label = i.label)), i.children && Yt(i.children, i), i.type === "space") {
        const s = i.width ?? 1;
        for (let l2 = 0; l2 < s; l2++) {
          const o = n(i);
          o.id = o.id + "-" + l2, F2.set(o.id, o), n2.push(o);
        }
...
var Te = F(() => {
  Tt.debug("Clear called"), Q2t(), et = { id: "root", type: "composite", children: [], columns: -1 }, F2 = /* @__PURE__ */ new Map([["root", et]]), St = [], lt = /* @__PURE__ */ new Map(), Lt = [], bt = /* @__PURE__ */ new Map();
}, "clear");
function Wt(e2) {
  switch (Tt.debug("typeStr2Type", e2), e2) {
    case "[]":
      return "square";
    case "()":
...
F(Kt, "edgeStrToEdgeData");
var Ct = 0;
var Ce = F(() => (Ct++, "id-" + Math.random().toString(36).substr(2, 12) + "-" + Ct), "generateId");
var Ie = F((e2) => {
...
var Oe = F(() => [...F2.values()], "getBlocksFlat");
var ze = F(() => St || [], "getBlocks");
var Re = F(() => Lt, "getEdges");
var Ae = F((e2) => F2.get(e2), "getBlock");
var Me = F((e2) => {
  F2.set(e2.id, e2);
}, "setBlock");
var Pe = F(() => Tt, "getLogger");
var Ye = F(function() {
  return lt;
}, "getClasses");
var We = {
  getConfig: F(() => ji().block, "getConfig"),
  typeStr2Type: Wt,
  edgeTypeStr2Type: Xt,
...
function ot(e2, t, a = 0, n2 = 0) {
  ...
  if (e2.children?.length > 0) {
    for (const b of e2.children)
      ot(b, t);
    const s = ar(e2);
    i = s.width, r = s.height, Tt.debug("setBlockSizes abc95 maxWidth of", e2.id, ":s children is ", i, r);
    for (const b of e2.children)
      b.size && (Tt.debug(
        `abc95 Setting size of children of ${e2.id} id=${b.id} ${i} ${r} ${JSON.stringify(b.size)}`
      ), b.size.width = i * (b.widthInColumns ?? 1) + C * ((b.widthInColumns ?? 1) - 1), b.size.height = r, b.size.x = 0, b.size.y = 0, Tt.debug(
    ...
    if (x < a) {
      ...
      Tt.debug("Size indata abc88", e2.id, "childWidth", b, "maxWidth", i), Tt.debug("Size indata abc88", e2.id, "childHeight", L, "maxHeight", r), Tt.debug("Size indata abc88 xSize", u, "padding", C);
      for (const S of e2.children)
        S.size && (S.size.width = b, S.size.height = L, S.size.x = 0, S.size.y = 0);
    }
    if (Tt.debug(
      `abc95 (finale calc) ${e2.id} xSize ${u} ySize ${h} columns ${l2}${e2.children.length} width=${Math.max(x, e2.size?.width || 0)}`
    ), x < (e2?.size?.width || 0)) {
      x = e2?.size?.width || 0;
      const b = l2 > 0 ? Math.min(e2.children.length, l2) : e2.children.length;
      if (b > 0) {
        const L = (x - b * C - C) / b;
        Tt.debug("abc95 (growing to fit) width", e2.id, x, e2.size?.width, L);
        for (const S of e2.children)
...
function kt(e2, t) {
  Tt.debug(
    `abc85 layout blocks (=>layoutBlocks) ${e2.id} x: ${e2?.size?.x} y: ${e2?.size?.y} width: ${e2?.size?.width}`
  );
  const a = e2.columns ?? -1;
  if (Tt.debug("layoutBlocks columns abc95", e2.id, "=>", a, e2), e2.children && // find max width of children
  e2.children.length > 0) {
    const n2 = e2?.children[0]?.size?.width ?? 0, i = e2.children.length * n2 + (e2.children.length - 1) * C;
    Tt.debug("widthOfChildren 88", i, "posX");
    let r = 0;
    Tt.debug("abc91 block?.size?.x", e2.id, e2?.size?.x);
    let s = e2?.size?.x ? e2?.size?.x + (-e2?.size?.width / 2 || 0) : -C, l2 = 0;
    for (const o of e2.children) {
      const u = e2;
      if (!o.size)
        continue;
      const { width: h, height: x } = o.size, { px: f, py: b } = Ft(a, r);
      if (b != l2 && (l2 = b, s = e2?.size?.x ? e2?.size?.x + (-e2?.size?.width / 2 || 0) : -C, Tt.debug("New row in layout for block", e2.id, " and child ", o.id, l2)), Tt.debug(
        `abc89 layout blocks (child) id: ${o.id} Pos: ${r} (px, py) ${f},${b} (${u?.size?.x},${u?.size?.y}) parent: ${u.id} width: ${h}${C}`
      ), u.size) {
        const L = h / 2;
        o.size.x = s + C + L, Tt.debug(
          `abc91 layout blocks (calc) px, pyid:${o.id} startingPos=X${s} new startingPosX${o.size.x} ${L} padding=${C} width=${h} halfWidth=${L} => x:${o.size.x} y:${o.size.y} ${o.widthInColumns} (width * (child?.w || 1)) / 2 ${h * (o?.widthInColumns ?? 1) / 2}`
        ), s = o.size.x + L, o.size.y = u.size.y - u.size.height / 2 + b * (x + C) + x / 2 + C, Tt.debug(
          `abc88 layout blocks (calc) px, pyid:${o.id}startingPosX${s}${C}${L}=>x:${o.size.x}y:${o.size.y}${o.widthInColumns}(width * (child?.w || 1)) / 2${h * (o?.widthInColumns ?? 1) / 2}`
        );
      }
      o.children && kt(o), r += o?.widthInColumns ?? 1, Tt.debug("abc88 columnsPos", o, r);
    }
  }
  Tt.debug(
    `layout blocks (<==layoutBlocks) ${e2.id} x: ${e2?.size?.x} y: ${e2?.size?.y} width: ${e2?.size?.width}`
  );
}
F(kt, "layoutBlocks");
...
function Ut(e2) {
  ...
  if (!t)
    return;
  ot(t, e2, 0, 0), kt(t), Tt.debug("getBlocks", JSON.stringify(t, null, 2));
...
F(ft, "applyStyle");
function jt(e2) {
  const t = gn(document.createElementNS("http://www.w3.org/2000/svg", "foreignObject")), a = t.append("xhtml:div"), n2 = e2.label, i = e2.isNode ? "nodeLabel" : "edgeLabel", r = a.append("span");
...
F(jt, "addHtmlLabel");
var sr = F(async (e2, t, a, n2) => {
  let i = e2 || "";
  if (typeof i == "object" && (i = i[0]), zr(_n().flowchart.htmlLabels)) {
    i = i.replace(/\\n|\n/g, "<br />"), Tt.debug("vertexText" + i);
    const r = await IG(kc(i)), s = {
      isNode: n2,
      label: r,
...
var M = {};
var lr = F(async (e2, t) => {
  const a = _n(), n2 = zr(a.flowchart.htmlLabels), i = t.labelType === "markdown" ? hl(
    e2,
    t.label,
    {
      style: t.labelStyle,
...
function tt(e2, t) {
  _n().flowchart.htmlLabels && e2 && (e2.style.width = t.length * 9 + "px", e2.style.height = "12px");
}
F(tt, "setTerminalWidth");
var or = F((e2, t) => {
  Tt.debug("Moving label abc88 ", e2.id, e2.label, wt[e2.id], t);
  let a = t.updatedPath ? t.updatedPath : t.originalPath;
  const n2 = _n(), { subGraphTitleTotalMargin: i } = U$(n2);
  if (e2.label) {
    const r = wt[e2.id];
    let s = e2.x, l2 = e2.y;
    if (a) {
      const o = as.calcLabelPosition(a);
      Tt.debug(
  ...
  if (e2.startLabelLeft) {
    ...
    let s = e2.x, l2 = e2.y;
    if (a) {
      const o = as.calcTerminalLabelPosition(e2.arrowTypeStart ? 10 : 0, "start_left", a);
      s = o.x, l2 = o.y;
    }
  ...
  if (e2.startLabelRight) {
    ...
    let s = e2.x, l2 = e2.y;
    if (a) {
      const o = as.calcTerminalLabelPosition(
  ...
  if (e2.endLabelLeft) {
    ...
    let s = e2.x, l2 = e2.y;
    if (a) {
      const o = as.calcTerminalLabelPosition(e2.arrowTypeEnd ? 10 : 0, "end_left", a);
      s = o.x, l2 = o.y;
    }
    r.attr("transform", `translate(${s}, ${l2})`);
  }
  if (e2.endLabelRight) {
    ...
    let s = e2.x, l2 = e2.y;
    if (a) {
      const o = as.calcTerminalLabelPosition(e2.arrowTypeEnd ? 10 : 0, "end_right", a);
      s = o.x, l2 = o.y;
    }
    r.attr("transform", `translate(${s}, ${l2})`);
  }
...
var dr = F((e2, t, a) => {
  ...
  if (Math.abs(i - t.y) * s > Math.abs(n2 - t.x) * o) {
    ...
    return l2 === 0 && (f.x = t.x, f.y = t.y), h === 0 && (f.x = t.x), u === 0 && (f.y = t.y), Tt.debug(`abc89 topp/bott calc, Q ${u}, q ${x}, R ${h}, r ${l2}`, f), f;
  } else {
    a.x < t.x ? l2 = t.x - s - n2 : l2 = n2 - s - t.x;
    let x = u * l2 / h, f = a.x < t.x ? a.x + h - l2 : a.x - h + l2, b = a.y < t.y ? a.y + x : a.y - x;
    return Tt.debug(`sides calc abc89, Q ${u}, q ${x}, R ${h}, r ${l2}`, { _x: f, _y: b }), l2 === 0 && (f = t.x, b = t.y), h === 0 && (f = t.x), u === 0 && (b = t.y), { x: f, y: b };
  }
}, "intersection");
var Bt = F((e2, t) => {
  Tt.debug("abc88 cutPathAtIntersect", e2, t);
...
var hr = F(function(e2, t, a, n2, i, r, s) {
  let l2 = a.points;
  Tt.debug("abc88 InsertEdge: edge=", a, "e=", t);
  let o = false;
  const u = r.node(t.v);
  var h = r.node(t.w);
  h?.intersect && u?.intersect && (l2 = l2.slice(1, a.points.length - 1), l2.unshift(u.intersect(l2[0])), l2.push(h.intersect(l2[l2.length - 1]))), a.toCluster && (Tt.debug("to cluster abc88", n2[a.toCluster]), l2 = Bt(a.points, n2[a.toCluster].node), o = true), a.fromCluster && (Tt.debug("from cluster abc88", n2[a.fromCluster]), l2 = Bt(l2.reverse(), n2[a.fromCluster].node).reverse(), o = true);
  const x = l2.filter((c) => !Number.isNaN(c.y));
  let f = A1;
  a.curve && (i === "graph" || i === "flowchart") && (f = a.curve);
  const { x: b, y: L } = m4t(a), S = Hvt().x(b).y(L).curve(f);
  let k;
  switch (a.thickness) {
    case "normal":
  ...
  let D = "";
  (_n().flowchart.arrowMarkerAbsolute || _n().state.arrowMarkerAbsolute) && (D = hW(true)), ir(T, a, D, s, i);
  let v = {};
...
F(Zt, "intersectEllipse");
var qt = Zt;
function Vt(e2, t, a) {
  return qt(e2, t, t, a);
}
F(Vt, "intersectCircle");
var pr = Vt;
function Gt(e2, t, a, n2) {
...
var R = F(async (e2, t, a, n2) => {
  const i = _n();
  let r;
  const s = t.useHtmlLabels || zr(i.flowchart.htmlLabels);
  ...
  let f = x.getBBox();
  const b = t.padding / 2;
  if (zr(i.flowchart.htmlLabels)) {
    const L = x.children[0], S = gn(x), k = L.getElementsByTagName("img");
    if (k) {
...
F(U, "insertPolygonShape");
var mr = F(async (e2, t) => {
  t.useHtmlLabels || _n().flowchart.htmlLabels || (t.centerLabel = true);
...
var Lr = mr;
var Ot = F((e2) => e2 ? " " + e2 : "", "formatClass");
var W = F((e2, t) => `${t || "node default"}${Ot(e2.classes)} ${Ot(
  e2.class
)}`, "getClassesFromNode");
var zt = F(async (e2, t) => {
  ...
  return o.attr("style", t.style), $(t, o), t.intersect = function(u) {
    return Tt.warn("Intersect called"), N.polygon(t, l2, u);
  }, a;
}, "question");
var Sr = F((e2, t) => {
...
var _r = F(async (e2, t) => {
  const { shapeSvg: a, bbox: n2 } = await R(e2, t, void 0, true), i = 2, r = n2.height + 2 * t.padding, s = r / i, l2 = n2.width + 2 * s + t.padding, o = ur(t.directions, n2, t), u = U(a, l2, r, o);
  return u.attr("style", t.style), $(t, u), t.intersect = function(h) {
    return N.polygon(t, o, h);
  }, a;
}, "block_arrow");
var Er = F(async (e2, t) => {
  ...
  return U(a, i, r, s).attr("style", t.style), t.width = i + r, t.height = r, t.intersect = function(l2) {
...
var Or = F(async (e2, t) => {
  ...
  if (n2.attr("width", i).attr("height", r), a.attr("class", "label edgeLabel"), t.props) {
    ...
    t.props.borders && (ct(n2, t.props.borders, i, r), s.delete("borders")), s.forEach((l2) => {
      Tt.warn(`Unknown node property ${l2}`);
    });
  }
...
function ct(e2, t, a, n2) {
  ...
  t.includes("t") ? (Tt.debug("add top border"), r(a)) : s(a), t.includes("r") ? (Tt.debug("add right border"), r(n2)) : s(n2), t.includes("b") ? (Tt.debug("add bottom border"), r(a)) : s(a), t.includes("l") ? (Tt.debug("add left border"), r(n2)) : s(n2), e2.attr("stroke-dasharray", i.join(" "));
}
F(ct, "applyNodePropertyBorders");
var zr2 = F(async (e2, t) => {
  let a;
  ...
  let o = "";
  typeof l2 == "object" ? o = l2[0] : o = l2, Tt.info("Label text abc79", o, l2, typeof l2 == "object");
  const u = s.node().appendChild(await K(o, t.labelStyle, true, true));
  let h = { width: 0, height: 0 };
  if (zr(_n().flowchart.htmlLabels)) {
    const S = u.children[0], k = gn(u);
...
var Ar = F(async (e2, t) => {
  ...
  return r.attr("style", t.style).attr("rx", t.rx).attr("ry", t.ry).attr("r", n2.width / 2 + i).attr("width", n2.width + t.padding).attr("height", n2.height + t.padding), Tt.info("Circle main"), $(t, r), t.intersect = function(s) {
    return Tt.info("Circle intersect", t, n2.width / 2 + i, s), N.circle(t, n2.width / 2 + i, s);
  }, a;
}, "circle");
var Mr = F(async (e2, t) => {
  const { shapeSvg: a, bbox: n2, halfPadding: i } = await R(
...
var Xr = F(async (e2, t) => {
  ...
  let b = 0;
  const L = t.classData.annotations?.[0], S = t.classData.annotations[0] ? "«" + t.classData.annotations[0] + "»" : "", k = f.node().appendChild(await K(S, t.labelStyle, true, true));
  let T = k.getBBox();
  if (zr(_n().flowchart.htmlLabels)) {
    const d = k.children[0], _ = gn(k);
    T = d.getBoundingClientRect(), _.attr("width", T.width), _.attr("height", T.height);
  }
  t.classData.annotations[0] && (x += T.height + n2, h += T.width);
  let D = t.classData.label;
  t.classData.type !== void 0 && t.classData.type !== "" && (_n().flowchart.htmlLabels ? D += "&lt;" + t.classData.type + "&gt;" : D += "<" + t.classData.type + ">");
  const v = f.node().appendChild(await K(D, t.labelStyle, true, true));
  gn(v).attr("class", "classTitle");
  let c = v.getBBox();
  if (zr(_n().flowchart.htmlLabels)) {
    const d = v.children[0], _ = gn(v);
    c = d.getBoundingClientRect(), _.attr("width", c.width), _.attr("height", c.height);
  }
  x += c.height + n2, c.width > h && (h = c.width);
  const p = [];
  t.classData.members.forEach(async (d) => {
    const _ = d.getDisplayDetails();
    let y2 = _.displayText;
    _n().flowchart.htmlLabels && (y2 = y2.replace(/</g, "&lt;").replace(/>/g, "&gt;"));
...
var nt = {};
var Qt = F(async (e2, t, a) => {
  let n2, i;
  if (t.link) {
    let r;
    _n().securityLevel === "sandbox" ? r = "_top" : t.linkTarget && (r = t.linkTarget || "_blank"), n2 = e2.insert("svg:a").attr("xlink:href", t.link).attr("target", r), i = await At[t.shape](n2, t, a);
  } else
...
function Et(e2, t, a = false) {
  const n2 = e2;
  let i = "default";
  (n2?.classes?.length || 0) > 0 && (i = (n2?.classes ?? []).join(" ")), i = i + " flowchart-label";
  let r = 0, s = "", l2;
  switch (n2.type) {
    case "round":
      r = 5, s = "rect";
  ...
  const o = hG(n2?.styles ?? []), u = n2.label, h = n2.size ?? { width: 0, height: 0, x: 0, y: 0 };
  return {
...
F(se, "insertEdges");
var Fr = F(function(e2, t) {
  return t.db.getClasses();
}, "getClasses");
var Ur = F(async function(e2, t, a, n2) {
  const { securityLevel: i, block: r } = ji(), s = n2.db;
  let l2;
  i === "sandbox" && (l2 = gn("#i" + t));
  const o = i === "sandbox" ? gn(l2.nodes()[0].contentDocument.body) : gn("body"), u = i === "sandbox" ? o.select(`[id="${t}"]`) : gn(`[id="${t}"]`);
  rr(u, ["point", "circle", "cross"], n2.type, t);
  const h = s.getBlocks(), x = s.getBlocksFlat(), f = s.getEdges(), b = u.insert("g").attr("class", "block");
  await re(b, h, s);
  const L = Ut(s);
  if (await ae(b, h, s), await se(b, f, x, s, t), L) {
    const S = L, k = Math.max(1, Math.round(0.125 * (S.width / S.height))), T = S.height + k + 10, D = S.width + 10, { useMaxWidth: v } = r;
    uW(u, T, D, !!v), Tt.debug("Here Bounds", L, S), u.attr(
      "viewBox",
...
//# sourceMappingURL=blockDiagram-6J76NXCF-CKU6xA4s-ur9z755R-XP4XVR7U.js.map
