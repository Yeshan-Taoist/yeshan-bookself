...
// node_modules/vitepress-plugin-legend/dist/kanban-definition-ZSS6B67P-CUcTB5vL-_rc-5uv9.js
var Z = (function() {
  var t = F(function(d, e, i, r) {
    ...
    performAction: F(function(d, e, i, r, l2, s, _) {
      var a = s.length - 1;
      switch (l2) {
        case 6:
        case 7:
    ...
    parse: F(function(d) {
      var e = this, i = [0], r = [], l2 = [null], s = [], _ = this.table, a = "", F2 = 0, ie = 0, he = 2, ne = 1, ue = s.slice.call(arguments, 1), m = Object.create(this.lexer), R = { yy: {} };
      for (var q in this.yy)
        Object.prototype.hasOwnProperty.call(this.yy, q) && (R.yy[q] = this.yy[q]);
      ...
      F(re, "lex");
      for (var S, B, D, Y, M = {}, W, v, oe, K; ; ) {
        if (B = i[i.length - 1], this.defaultActions[B] ? D = this.defaultActions[B] : ((S === null || typeof S > "u") && (S = re()), D = _[B] && _[B][S]), typeof D > "u" || !D.length || !D[0]) {
          var J = "";
          K = [];
          for (W in _[B])
            this.terminals_[W] && W > he && K.push("'" + this.terminals_[W] + "'");
          m.showPosition ? J = "Parse error on line " + (F2 + 1) + `:
` + m.showPosition() + `
Expecting ` + K.join(", ") + ", got '" + (this.terminals_[S] || S) + "'" : J = "Parse error on line " + (F2 + 1) + ": Unexpected " + (S == ne ? "end of input" : "'" + (this.terminals_[S] || S) + "'"), this.parseError(J, {
            text: m.match,
        ...
        switch (D[0]) {
          case 1:
            i.push(S), l2.push(m.yytext), s.push(m.yylloc), i.push(D[1]), S = null, ie = m.yyleng, a = m.yytext, F2 = m.yylineno, z = m.yylloc;
            break;
          case 2:
...
` + this.showPosition(), {
      ...
      performAction: F(function(e, i, r, l2) {
        switch (r) {
          case 0:
  ...
  return F(P, "Parser"), P.prototype = H, H.Parser = P, new P();
})();
Z.parser = Z;
var Ne = Z;
var L = [];
...
var xe = F(function() {
  ...
  return { nodes: u, edges: t, other: {}, config: _n() };
}, "getData");
var Oe = F((t, u, p, n, g) => {
  const f = _n();
  let E = f.mindmap?.padding ?? eW.mindmap.padding;
  switch (n) {
    case y.ROUNDED_RECT:
    case y.RECT:
    case y.HEXAGON:
...
` + g + `
    ...
    h?.shape && h.shape === "kanbanItem" && (c.shape = h?.shape), h?.label && (c.label = h?.label), h?.icon && (c.icon = h?.icon.toString()), h?.assigned && (c.assigned = h?.assigned.toString()), h?.ticket && (c.ticket = h?.ticket.toString()), h?.priority && (c.priority = h?.priority);
  }
  const x = Le(t);
  x ? c.parentId = x.id || "kbn" + Q++ : te.push(c), L.push(c);
}, "addNode");
var y = {
  DEFAULT: 0,
  NO_BORDER: 0,
...
var Ie = F((t, u) => {
  switch (Tt.debug("In get type", t, u), t) {
    case "[":
      return y.RECT;
    case "(":
...
var Ae = F((t) => {
  if (!t)
    return;
  const u = _n(), p = L[L.length - 1];
  t.icon && (p.icon = sc(t.icon, u)), t.class && (p.cssClasses = sc(t.class, u));
}, "decorateNode");
var we = F((t) => {
  switch (t) {
    case y.DEFAULT:
...
var Ce = F(() => Tt, "getLogger");
var $e = F((t) => se[t], "getElementById");
var Te = {
  clear: De,
  addNode: Oe,
  getSections: le,
...
var Re = Te;
var Be = F(async (t, u, p, n) => {
  Tt.debug(`Rendering kanban diagram
` + t);
  const g = n.db.getData(), f = _n();
  f.htmlLabels = false;
  const E = p9t(u), c = E.append("g");
  ...
  let T = 25;
  for (const b of I) {
    const A = f?.kanban?.sectionWidth || 200;
    h = h + 1, b.x = A * h + (h - 1) * $ / 2, b.width = A, b.y = 0, b.height = A * 3, b.rx = 5, b.ry = 5, b.cssClasses = b.cssClasses + " section-" + h;
    const w = await v8t(c, b);
    T = Math.max(T, w?.labelBBox?.height), k.push(w);
  }
  let O = 0;
  for (const b of I) {
    const A = k[O];
    O = O + 1;
    const w = f?.kanban?.sectionWidth || 200, C = -w * 3 / 2 + T;
    let G = C;
    ...
    for (const d of H) {
      ...
      const e = (await lY(x, d, { config: f })).node().getBBox();
      d.y = G + e.height / 2, await Jre(d), G = d.y + e.height / 2 + $ / 2;
    }
    const X = A.cluster.select("rect"), P = Math.max(G - C + 3 * $, 50) + (T - 25);
    X.attr("height", P);
  }
  X2t(
    void 0,
    E,
    f.mindmap?.padding ?? eW.kanban.padding,
    f.mindmap?.useMaxWidth ?? eW.kanban.useMaxWidth
  );
}, "draw");
var Ge = {
  draw: Be
...
var Pe = F((t) => {
  ...
  for (let n = 0; n < t.THEME_COLOR_LIMIT; n++)
    t["lineColor" + n] = t["lineColor" + n] || t["cScaleInv" + n], uf(t["lineColor" + n]) ? t["lineColor" + n] = fe(t["lineColor" + n], 20) : t["lineColor" + n] = _e(t["lineColor" + n], 20);
  const p = F((n, g) => t.darkMode ? _e(n, g) : fe(n, g), "adjuster");
  for (let n = 0; n < t.THEME_COLOR_LIMIT; n++) {
    const g = "" + (17 - 3 * n);
    u += `
...
//# sourceMappingURL=kanban-definition-ZSS6B67P-CUcTB5vL-_rc-5uv9-V4EFNRYH.js.map
